    ...
    <div class="modal-dialog modal-dialog-centered modal-simple">
        <div class="modal-content">
            <div class="modal-body">
                ...
                <form action="{{ route('admin.product-categories.store') }}" method="POST" class="row">
                    @csrf
                    <div class="col-12 form-control-validation mb-4">
                        <label class="form-label" for="prototypeSelect">Select Prototype</label>
                        <select id="prototypeSelect" name="prototype_id" class="form-control" required>
                            <option value="">-- Select Prototype --</option>
                            @foreach ($prototype as $prototypes)
                                <option value="{{ $prototypes->id }}" 
                                    {{ old('prototype_id', $selectedPrototypeId ?? '') == $prototypes->id ? 'selected' : '' }}>
                                    {{ $prototypes->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-12 text-center demo-vertical-spacing">
                        <button type="submit" class="btn btn-primary me-sm-4 me-1">Create Category</button>
